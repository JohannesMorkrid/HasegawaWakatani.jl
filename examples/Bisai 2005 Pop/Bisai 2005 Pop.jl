...
domain = Domain(128, 128; Lx=160, Ly=160, MemoryType=CuArray, precision=Float64)
...
# Parameters
parameters = (μ=1e-2, ν=1e-2, g=8e-4, σ_0=2e-4, S_0=5e-4, λ_s=5.0, λ_θ=10)

tspan = [0.0, 5_000_000.0]
...
function source(x, y, S_0, λ_s, λ_θ)
    @. S_0 * (0.1 * sin(λ_θ * y) + 1) * exp(-((x + 80 - λ_s / 2) / λ_s)^2) + 0 * y
end

S = get_fwd(domain) * CuArray(source(domain.x', domain.y, 5e-4, 60, 30))
using Plots
...
prob = SpectralODEProblem(Linear, NonLinear, ic, domain, tspan; p=parameters, dt=1,
                          ...
                          #additional_operators=[OperatorRecipe(:constant_source;
                          #shape=source)]
                          )
...
density = output.simulation["Density/data"][:, :, 1:3000]
plot(mean(mean(density; dims=3); dims=1)[:])